{ self, inputs, ... }: {
  ...
  let
    ...
    zenUserPrefs = {
      ...
      "theme.custom_uifont.custom" = "JetBrains Mono";
  ...
  in {
    ...
    home.activation.zenBrowserConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      fetch_if_missing() {
        [ -f "$1" ] || $DRY_RUN_CMD curl -fsSL --retry 2 -o "$1" "$2" || true
      }

      PROFILE_DIR=""
      for ZEN_BASE in "$HOME/.zen" "$HOME/.var/app/app.zen_browser.zen/.zen"; do
        [ -f "$ZEN_BASE/profiles.ini" ] || continue
        ZEN_REL="$(awk '/^\[Install/{f=1;next} /^\[/{f=0} f && /^Default=/{sub(/^Default=/,""); print; exit}' "$ZEN_BASE/profiles.ini")"
        if [ -n "$ZEN_REL" ] && [ -d "$ZEN_BASE/$ZEN_REL" ]; then
          PROFILE_DIR="$ZEN_BASE/$ZEN_REL"
          break
        fi
      done
      ...
      if [ -n "$PROFILE_DIR" ]; then
        ...
        if $DRY_RUN_CMD curl -fsSL --retry 2 -o "$ZEN_MODS_INDEX" "${zenModsIndexUrl}"; then
